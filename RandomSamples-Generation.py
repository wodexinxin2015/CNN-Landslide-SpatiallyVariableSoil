...
# input the initial file folder
proj_path = input("Input the directory of input.dat: (where is the input.dat) \n")
if os.sep == "/":
            ...
            for i in range(0, cell_total):
                for j in range(0, cell_total):
                    ...
                    if randf_para[2] == 1:
                        ...
                        acf = np.exp(-np.pi * (np.pow(dx[0] / lx, 2) + np.pow(dx[1] / ly, 2) + np.pow(dx[2] / lz, 2)))
